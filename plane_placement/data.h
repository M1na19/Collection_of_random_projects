...
Airplane GenerateRandomAirplane(int nrRows,int nrSeats){
    ...
    a.groups=smartstore<Group>();
    
    for(int i=0;i<nrGroups;i++){
        Group newG;
        a.groups.pushBack(newG);
    }

    for(int i=0;i<nrRows;i++){
        for(int j=0;j<nrSeats;j++){
            //find random group
            int randomGroup=rand()%nrGroups;

            //give it the details
            Passanger current;
            current.row=i+1;
            current.seat=char('A'+j);
            current.groupID=randomGroup+1;
            current.waitTime=rand()%(MAX_WAIT_TIME-MIN_WAIT_TIME+1)+MIN_WAIT_TIME;
            
            //give it away
            a.groups[randomGroup].passangers.pushBack(current);//CHECK
        }
    }
    return a;
}
